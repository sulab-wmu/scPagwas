...
```{r message=FALSE, warning=FALSE}
scPagwas_topgenes <- names(Pagwas$gene_heritability_correlation[
  order(Pagwas$gene_heritability_correlation, decreasing = T), ])[1:1000]
Single_data <- Seurat::AddModuleScore(Single_data, assay = "RNA", list(scPagwas_topgenes), name = c("scPagwas.TRS.Score"))
#run pvalue
CellScalepValue <- scGene_rankP(Single_mat = t(data.matrix(Seurat::GetAssayData(Single_data, assay = "RNA")[scPagwas_topgenes, ])))
```
