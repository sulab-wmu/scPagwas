...
Bootstrap_P_Barplot <- function(Pagwas, title = "Test scPagwas",
  ...
  if (sum(cell_severe$bp_value < 0.05) > 0) {
    p1 <- ggplot2::ggplot(cell_severe, aes(x = reorder(annotation, logp), y = logp, fill = sig)) +
    ...
    p1 <- ggplot2::ggplot(cell_severe, aes(x = reorder(annotation, logp), y = logp)) +
...
Bootstrap_estimate_Plot <- function(Pagwas,
  ...
  plot1 <- ggplot2::ggplot(dat, aes(y = Index, x = estimate)) +
