...
scPagwas_perform_score <- function(Pagwas,
  ...
  Pathway_sclm_results<-lapply(Pathway_names,function(Pathway){
    ...
    if(!is.null(Pathway_block$x)){
      if(nrow(Pathway_block$x)>2){
        na_elements <- is.na(Pathway_block$y) | apply(Pathway_block$x, 1, function(x) { any(is.na(x))}) | is.na(noise_per_snp)
        x<-Pathway_block$x
  ...
  Pathway_sclm_results2<-as.data.frame(lapply(Pathway_sclm_results,function(x){x[is.na(x)]<-0;return(x)}))
  ...
  if(remove_outlier){Pagwas$scPagwas_score<-scPagwas_score_filter(scPagwas_score=Pagwas$scPagwas_score)}
