...
plot_scpathway_dot<-function(Pagwas,
  ...
  #library("rhdf5")
###############proportion
  proportion_list<-tapply(Pagwas$Celltype_anno$cellnames,
         as.factor(Pagwas$Celltype_anno$annotation),function(x){
    a<-apply(Pagwas$Pathway_sclm_results[x,],2,function(y) sum(y>0)/length(y))
    return(unlist(a))
  })
...
##############rankp value
  ...
  top_function <-function(para_mat,n_path_to_keep=5){
    ...
    d_spe <- d_spe %>% group_by(celltypes) %>% top_n(.,n_path_to_keep,specificity)
    return(d_spe)
  }
  ...
  p<-plot_scpathway_contri_dot(data.to.plot = gg_dot,
                           display_max_sizes=F,
                           size_var = size_var,
                           col_var=col_var,
                           shape.scale = 8,
